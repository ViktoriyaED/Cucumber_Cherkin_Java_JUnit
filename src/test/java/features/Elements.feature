...
#    And User should see a confirmation message containing "Permanent Address:456 Oak Ave"
  ...
  Scenario Outline: Multiple Users fill out form and submits successfully
    When User clicks on the Text Box side menu
    And User fill out the Full Name field with "<fullName>"
    And User fill out the Email field with "<email>"
    And User fill out the Current Address field with "<currentAddress>"
    And User fill out the Permanent Address field with "<permanentAddress>"
    And User clicks on the Submit button
    Examples:
    |fullName  |email            |currentAddress|permanentAddress|
    |John Doe  |john@example.com |123 Main St   |456 Oak Ave     |
    |Mary Bri  |mary@example.com |321 Home St   |654 Tree Ave    |
